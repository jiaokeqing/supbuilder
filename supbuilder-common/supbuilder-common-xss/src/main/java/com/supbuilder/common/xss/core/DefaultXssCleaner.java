...
import com.supbuilder.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final SupbuilderXssProperties properties;

	public DefaultXssCleaner(SupbuilderXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(SupbuilderXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		SupbuilderXssProperties.Mode mode = properties.getMode();
		if (SupbuilderXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (SupbuilderXssProperties.Mode.validate == mode) {
			// 校验
